...
export default function Team() {
  ...
  const categories = [
    { name: "Miotacze", position: "Miotacz" },
    { name: "Łapacze", position: "Łapacz" },
    { name: "Gracze z pola", position: "Gracz z pola" },
    { name: "Zapolowi", position: "Zapolowy" },
  ];
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="space-y-16">
          {categories.map((category) => {
            const categoryPlayers = players.filter(p => p.position === category.position);
            if (categoryPlayers.length === 0) return null;
            return (
              <div key={category.name}>
                <h2 className="text-3xl font-display font-bold uppercase text-secondary mb-8 flex items-center gap-4">
